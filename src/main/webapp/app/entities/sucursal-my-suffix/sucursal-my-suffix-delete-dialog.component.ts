import { Component, OnInit, OnDestroy } from '@angular/core';
import { ActivatedRoute } from '@angular/router';

import { NgbActiveModal } from '@ng-bootstrap/ng-bootstrap';
import { JhiEventManager } from 'ng-jhipster';

import { SucursalMySuffix } from './sucursal-my-suffix.model';
import { SucursalMySuffixPopupService } from './sucursal-my-suffix-popup.service';
import { SucursalMySuffixService } from './sucursal-my-suffix.service';

@Component({
    selector: 'jhi-sucursal-my-suffix-delete-dialog',
    templateUrl: './sucursal-my-suffix-delete-dialog.component.html'
})
export class SucursalMySuffixDeleteDialogComponent {

    sucursal: SucursalMySuffix;

    constructor(
        private sucursalService: SucursalMySuffixService,
        public activeModal: NgbActiveModal,
        private eventManager: JhiEventManager
    ) {
    }

    clear() {
        this.activeModal.dismiss('cancel');
    }

    confirmDelete(id: number) {
        this.sucursalService.delete(id).subscribe((response) => {
            this.eventManager.broadcast({
                name: 'sucursalListModification',
                content: 'Deleted an sucursal'
            });
            this.activeModal.dismiss(true);
        });
    }
}

@Component({
    selector: 'jhi-sucursal-my-suffix-delete-popup',
    template: ''
})
export class SucursalMySuffixDeletePopupComponent implements OnInit, OnDestroy {

    routeSub: any;

    constructor(
        private route: ActivatedRoute,
        private sucursalPopupService: SucursalMySuffixPopupService
    ) {}

    ngOnInit() {
        this.routeSub = this.route.params.subscribe((params) => {
            this.sucursalPopupService
                .open(SucursalMySuffixDeleteDialogComponent as Component, params['id']);
        });
    }

    ngOnDestroy() {
        this.routeSub.unsubscribe();
    }
}
