...
@Component({
    selector: 'jhi-carro-my-suffix',
    templateUrl: './carro-my-suffix.component.html'
})
export class CarroMySuffixComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.carroService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<CarroMySuffix[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/carro-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/carro-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: CarroMySuffix) {
        return item.id;
    }
