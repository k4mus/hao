...
@Component({
    selector: 'jhi-entrega-my-suffix',
    templateUrl: './entrega-my-suffix.component.html'
})
export class EntregaMySuffixComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.entregaService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<EntregaMySuffix[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/entrega-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/entrega-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: EntregaMySuffix) {
        return item.id;
    }
