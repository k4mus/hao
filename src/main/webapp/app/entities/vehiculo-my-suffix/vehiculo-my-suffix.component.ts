...
@Component({
    selector: 'jhi-vehiculo-my-suffix',
    templateUrl: './vehiculo-my-suffix.component.html'
})
export class VehiculoMySuffixComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.vehiculoService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<VehiculoMySuffix[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/vehiculo-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/vehiculo-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: VehiculoMySuffix) {
        return item.id;
    }
