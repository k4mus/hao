...
@Component({
    selector: 'jhi-sucursal-my-suffix',
    templateUrl: './sucursal-my-suffix.component.html'
})
export class SucursalMySuffixComponent implements OnInit, OnDestroy {
...
currentAccount: any;
    ...
    loadAll() {
        this.sucursalService.query({
            ...
            sort: this.sort()}).subscribe(
                (res: HttpResponse<SucursalMySuffix[]>) => this.onSuccess(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/sucursal-my-suffix'], {queryParams:
            {
                page: this.page,
                size: this.itemsPerPage,
    ...
    clear() {
        this.page = 0;
        this.router.navigate(['/sucursal-my-suffix', {
            page: this.page,
            sort: this.predicate + ',' + (this.reverse ? 'asc' : 'desc')
        }]);
        this.loadAll();
    ...
    trackId(index: number, item: SucursalMySuffix) {
        return item.id;
    }
